...
class NodeAB():
    def __init__(self,raiz,nombre,calificacion):
        self.raiz = raiz
        self.nombre = nombre
        self.calificacion = calificacion
        self.der = None
...
class ArbolB: 
    ...
    def insertar(self,dato,nombre,calificacion):
        if not self.raiz:
            self.raiz = NodeAB(dato,nombre,calificacion)
        else:
            self._insertar_rec(self.raiz,dato,nombre,calificacion)

    def _insertar_rec(self,actual,dato,nombre,calificacion):
        if actual is not None:
            if actual.raiz > dato:
                if actual.izq is None:
                    actual.izq = NodeAB(dato,nombre,calificacion)
                else:
                    self._insertar_rec(actual.izq,dato,nombre,calificacion)
            else:
                if actual.der is None:
                    actual.der = NodeAB(dato,nombre,calificacion)
                else:
                    self._insertar_rec(actual.der,dato,nombre,calificacion)
...
class LinkedList:
    ...
    def agregar_estudiante(self, id_estudiante, nombre, calificacion):
        ...
        print(f"\nEstudiante {nombre} añadido al final.")
...
class Queue:
    ...
    def remove(self):
        if self.isEmpty():
            ...
            print(f'Alumno {self.list[0]} atendido')
            self.list.pop(0)
...
def data():
    x = input('Quiere acceder al reporte alfabeticamente(A) o por calificaciones(B)? ').lower().strip()
    if x == 'a':
        try:
            with open('ReporteAlfabetico.txt', 'r') as f:
                for line in f:
                    line.replace('\n', '')
                    print(line)
            return True
        ...
        try:
            with open('ReporteCalificaciones.txt', 'r') as f:
                for line in f:
                    line.replace('\n', '')
                    print(line)
            return True
...
def main():
    ...
    sets = set() #Set para no repetir el id del alumno
    while True:
        x = menu()
        match x:
            case '1':
                id_alum = (input('Ingrese ID del estudiante: '))
                if id_alum in sets:
                    print('El id que usted puso ya esta tomado.')
                    continue
                else:
                    sets.add(id_alum)
                    id = int(id_alum)
                    ...
                    lista_estudiantes.agregar_estudiante(id, nom_alum, cal_alum)
                    cola.add(nom_alum)
                    tree.insertar(id,nom_alum,cal_alum)

            case '2':
                if not cola.isEmpty():
                    print(f"Atendiendo al alumno: {cola.list[0]}")
                    cola.remove()
                else:
            ...
            case '4':
                    x = input('Quiere guardar el reporte alfabeticamente(A) o por calificaciones(B)? ').lower().strip()
                    if x == 'a':
